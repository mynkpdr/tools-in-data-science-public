...
⏱ ~6–8 hours
🔗 needs: [Speech AI](2026-02/docs/week-6/speech-ai.md) · [FastAPI](2026-02/docs/week-2/01-fastapi.md) · [Structured Output](2026-02/docs/week-3/structured-output.md)
...
**3. STT with confidence.** Use [faster-whisper](2026-02/docs/week-6/speech-ai.md) (or equivalent). Surface the detected language and its probability. If confidence is low, say so in the response rather than confidently mistranslating noise.

**4. Translate.** Any LLM or translation API, with a schema-validated response ([Structured Output](2026-02/docs/week-3/structured-output.md)) — never a raw free-text blob you then regex.
...
**6. Engineer for failure.** Every stage gets a timeout. A stage failure returns a clear error naming *which* stage failed, never a 500 with a stack trace. Log per-stage latency with a request ID ([observability](2026-02/docs/week-2/09-observability.md)).
...
| 2 | Deployed endpoint ([Cloud Run](2026-02/docs/week-7/07-serverless-functions.md) or similar) — or a recorded demo if deployment isn't feasible |
...
| Timeout on long audio | Whole file in one request | Cap duration; chunk; consider a [queue](2026-02/docs/week-7/10-pubsub-event-driven.md) |
